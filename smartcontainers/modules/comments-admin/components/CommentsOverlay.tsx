...
// ============================================================
// comments-admin module — CommentsOverlay
//
// Props:
//   currentView  — string identyfikujący bieżący widok/stronę
//   apiUrl       — opcjonalny URL API (przekazywany do useComments)
//   isMobile     — czy renderować w trybie mobilnym (zmienia pozycję przycisku)
// ============================================================
...
const CommentsOverlay: React.FC<CommentsOverlayProps> = ({
  ...
  const { comments, addComment, deleteComment, isLoading } = useComments(
    currentView,
    apiUrl,
  );
  ...
  // Stan nowo tworzonego komentarza
  ...
  return (
    <>
      ...
      {/* Nakładka łapiąca kliknięcia */}
      <div
        className={`absolute inset-0 z-40 ${
          isCommentingMode ? 'cursor-crosshair bg-black/5' : 'pointer-events-none'
        }`}
        ...
        {!isLoading &&
          comments.map(comment => (
            ...
            >
              ...
              {(comment.isAdminComment
                ? !collapsedAdminComments.has(comment.id)
                : activeCommentId === comment.id) && (
                <div
                  className={`absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 rounded-lg shadow-xl border p-3 text-sm flex flex-col gap-2 ${
                    comment.isAdminComment ? 'bg-red-50 border-red-200' : 'bg-white border-gray-100'
                  }`}
                >
                  ...
                  <p className={`${comment.isAdminComment ? 'text-red-900' : 'text-gray-600'} break-words flex-1`}>
                    {comment.text}
                  </p>
